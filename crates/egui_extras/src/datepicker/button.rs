use super::{popup::DatePickerPopup, YearRange};
...
impl<'a> DatePickerButton<'a> {
  ...
  pub fn year_range(mut self, range: YearRange) -> Option<Self> {
    NaiveDate::from_ymd_opt(range[0], 1, 1)?;
    NaiveDate::from_ymd_opt(range[1], 1, 1)?;
...
impl<'a> Widget for DatePickerButton<'a> {
  fn ui(self, ui: &mut Ui) -> egui::Response {
    ...
    let mut button_state = ui.memory_mut(|mem| mem.data.get_persisted::<DatePickerButtonState>(id)).unwrap_or_default();
    ...
    if button_state.picker_visible {
      ...
      let width_with_padding = width + ui.style().spacing.item_spacing.x + ui.style().spacing.window_margin.left + ui.style().spacing.window_margin.right;
      ...
      let InnerResponse { inner: saved, response: area_response } = Area::new(ui.make_persistent_id(self.id_source)).order(Order::Foreground).fixed_pos(pos).show(ui.ctx(), |ui| {
