...
use super::{button::DatePickerButtonState, month_data, YearRange};
...
impl DatePickerPopupState {
  fn last_day_of_month(&self) -> u32 {
    let date: NaiveDate = NaiveDate::from_ymd_opt(self.year, self.month, 1).expect("Could not create NaiveDate");
    date.with_day(31).map(|_| 31).or_else(|| date.with_day(30).map(|_| 30)).or_else(|| date.with_day(29).map(|_| 29)).unwrap_or(28)
  }
}
...
impl<'a> DatePickerPopup<'a> {
  ...
  pub fn draw(&mut self, ui: &mut Ui) -> bool {
    ...
    let mut popup_state = ui.memory_mut(|mem| mem.data.get_persisted::<DatePickerPopupState>(id)).unwrap_or_default();
    ...
    StripBuilder::new(ui)
      ...
      .sizes(Size::exact((spacing + height) * (weeks.len() + 1) as f32), self.calendar as usize)
        ...
        if self.combo_boxes {
          strip.strip(|builder| {
            builder.sizes(Size::remainder(), 3).horizontal(|mut strip| {
              strip.cell(|ui| {
                ComboBox::from_id_source("date_picker_year").selected_text(popup_state.year.to_string()).show_ui(ui, |ui| {
                  for year in self.year_range[0]..=self.year_range[1] {
                    if ui.selectable_value(&mut popup_state.year, year, year.to_string()).changed() {
                      popup_state.day = popup_state.day.min(popup_state.last_day_of_month());
                      ui.memory_mut(|mem| {
              ...
              strip.cell(|ui| {
                ComboBox::from_id_source("date_picker_month").selected_text(month_name(popup_state.month)).show_ui(ui, |ui| {
                  for month in 1..=12 {
                    if ui.selectable_value(&mut popup_state.month, month, month_name(month)).changed() {
                      popup_state.day = popup_state.day.min(popup_state.last_day_of_month());
                      ui.memory_mut(|mem| {
                        mem.data.insert_persisted(id, popup_state.clone());
                      });
              ...
              strip.cell(|ui| {
                ComboBox::from_id_source("date_picker_day").selected_text(popup_state.day.to_string()).show_ui(ui, |ui| {
                  for day in 1..=popup_state.last_day_of_month() {
                    if ui.selectable_value(&mut popup_state.day, day, day.to_string()).changed() {
                      ui.memory_mut(|mem| {
                        mem.data.insert_persisted(id, popup_state.clone());
                      });
                    }
            ...
            builder.sizes(Size::remainder(), 6).horizontal(|mut strip| {
              strip.cell(|ui| {
                ui.with_layout(Layout::top_down_justified(Align::Center), |ui| {
                  if ui.button("<<<").on_hover_text("subtract one year").clicked() {
                    popup_state.year -= 1;
                    popup_state.day = popup_state.day.min(popup_state.last_day_of_month());
                    ui.memory_mut(|mem| {
                      mem.data.insert_persisted(id, popup_state.clone());
              ...
              strip.cell(|ui| {
                ui.with_layout(Layout::top_down_justified(Align::Center), |ui| {
                  if ui.button("<<").on_hover_text("subtract one month").clicked() {
                    popup_state.month -= 1;
                ...
                for week in weeks {
                  body.row(height, |mut row| {
                    ...
                    for day in week.days {
                      row.col(|ui| {
                        ui.with_layout(Layout::top_down_justified(Align::Center), |ui| {
                          let fill_color = if popup_state.year == day.year() && popup_state.month == day.month() && popup_state.day == day.day() {
                            ui.visuals().selection.bg_fill
                          } else if day.weekday() == Weekday::Sat || day.weekday() == Weekday::Sun {
                          ...
                          let button_response = ui.add(Button::new(RichText::new(day.day().to_string()).color(text_color)).fill(fill_color));

                          if day == today {
                            // Encircle today's date
                            let stroke = ui.visuals().widgets.inactive.fg_stroke;
                            ui.painter().circle_stroke(button_response.rect.center(), 8.0, stroke);
                          }
            ...
            strip.cell(|ui| {
              ui.with_layout(Layout::top_down_justified(Align::Center), |ui| {
                if ui.button("Save").clicked() {
                  *self.selection = NaiveDate::from_ymd_opt(popup_state.year, popup_state.month, popup_state.day).expect("Could not create NaiveDate");
                  saved = true;
                  close = true;
                }
              });
    ...
    if close {
      ...
      ui.memory_mut(|mem| {
        ...
        mem.data.get_persisted_mut_or_default::<DatePickerButtonState>(self.button_id).picker_visible = false;
      });
    }
